  ...
  $.fn.smartAutoComplete = function(){    
    ...
    $.event.special.itemFocus = {
      ...
      _default: function(ev){    
		...
		$(item).click(function() {
  			var n_map = getMap();
			var jqName = item.innerHTML;
			var jqFuid = n_map[jqName];
			myId = jqFuid;
			if (n_map["isConnected-"+jqFuid] == "false" || n_map["isConnected-"+jqFuid] == undefined) { 
				FB.api("/"+jqFuid+"/friends?fields=id,name,picture", handleMyFriends); 
				updateMap("isConnected-"+jqFuid, "true"); 
			} else { 
				FB.api("/"+jqFuid+"/friends?fields=id,name,picture", handleRemoveMyFriends); 
				updateMap("isConnected-"+jqFuid, "false"); 
			}
			//FB.api("/"+jqFuid+"/friends?fields=id,name,picture", handleMyFriends); 
		});
    ...
    return this.each(function(i) { 
      ...
      if($.type(options.resultsContainer) === 'undefined' ){
        ...
        var default_container = $("<ul id='queryResult' class='smart_autocomplete_container' style='display:none'></ul>");
